export const galleries = {
  home: {
    name: 'Home',
    description: 'Sunsets from Oahu',
    photos: [
      { 
        id: 1, 
        url: '/photos/oahu/firecracker.JPG', 
        alt: 'Firecracker',
        caption: 'Firecracker flower, I\'m not sure what the actual name is.'
      },
      { 
        id: 2, 
        url: '/photos/oahu/pearl.JPG', 
        alt: 'Pearl Harbor',
        caption: 'Red sky above Pearl Harbor.'
      },
      { 
        id: 3, 
        url: '/photos/oahu/pinch.JPG', 
        alt: 'Pinch',
        caption: 'My brother tries to capture the sun.'
      },
      { 
        id: 4, 
        url: '/photos/oahu/papaya.JPG', 
        alt: 'Papaya',
        caption: 'Papaya tree is too tall.'
      },
      { 
        id: 5, 
        url: '/photos/oahu/ohia.JPG', 
        alt: 'Ohia Lehua',
        caption: '\'Ohi\'a Lehua on the Wa\'ahila Ridge.'
      },
      { 
        id: 6, 
        url: '/photos/oahu/rainbow.JPG', 
        alt: 'Rainbow',
        caption: 'The rainbow will be the sign of that solemn promise.'
      },
      { 
        id: 7, 
        url: '/photos/oahu/stairway.JPG', 
        alt: 'Stairway to Heaven',
        caption: 'The Stairway to Heaven. I didn\'t think we were going to make it.'
      },
      { 
        id: 8, 
        url: '/photos/oahu/wave.JPG', 
        alt: 'Wave Crashing',
        caption: 'Wave crashing on the Ka\'iwi Shoreline.'
      },
      { 
        id: 9, 
        url: '/photos/oahu/climb.JPG', 
        alt: 'Rock Climbing',
        caption: 'Be careful along the edges.'
      }
    ]
  },
  lyon: {
    name: 'Lyon',
    description: '5.17 - 7.30.25 | NSF Summer Research Program in Lyon, France',
    photos: [
      { 
        id: 1, 
        url: '/photos/lyon/flag.JPG', 
        alt: 'French Flag',
        caption: 'Walking to the train station on the way to work.'
      },
      { 
        id: 2, 
        url: '/photos/lyon/palais_bourse.JPG', 
        alt: 'Palais de la Bourse de Lyon',
        caption: 'Fancy party at Palais de la Bourse de Lyon. I was\'nt invited.'
      },
      { 
        id: 3, 
        url: '/photos/lyon/place_jacobins.JPG', 
        alt: 'Place des Jacobins',
        caption: 'Peaceful Place des Jacobins. Good bakery nearby, Les Pain des Jacobins.'
      },
      { 
        id: 4, 
        url: '/photos/lyon/aynaz.JPG', 
        alt: 'Aynaz',
        caption: 'Aynaz in front of Parc de la Tete d\'Or.'
      },
      { 
        id: 5, 
        url: '/photos/lyon/saone.JPG', 
        alt: 'River Saone',
        caption: 'Buildings rise on the hill behind the river Saone.'
      },
      { 
        id: 6, 
        url: '/photos/lyon/vieux_lyon.JPG', 
        alt: 'Vieux Lyon',
        caption: 'Vieux Lyon from Place Bellecour. There are many churches.'
      }
    ]
  },
  annecy: {
    name: 'Lyon',
    description: '5.29 - 6.1.25 | Annecy changed my life',
    photos: [
      { 
        id: 1, 
        url: '/photos/annecy/paraglider.JPG', 
        alt: 'Paraglider',
        caption: 'Paragliding in Doussard, on the other side of Lac Annecy.'
      },
      { 
        id: 2, 
        url: '/photos/annecy/mountain.JPG', 
        alt: 'Sunset Mountain',
        caption: 'Sunset over the mountains while chilling on the lake.'
      },
      { 
        id: 3, 
        url: '/photos/annecy/church.JPG', 
        alt: 'Church',
        caption: 'Basilique de la Visitation. I think Henry went to mass there.'
      },
      { 
        id: 4, 
        url: '/photos/annecy/landing.JPG', 
        alt: 'Aynaz Landing',
        caption: 'Aynaz went paragliding, but I was too scared. I regret it.'
      },
      { 
        id: 5, 
        url: '/photos/annecy/bike.JPG', 
        alt: 'Bikers',
        caption: 'There was a big bike race. We biked around the lake, too, but not in the race. 26 miles... Sorry mom & dad :)'
      },
      { 
        id: 6, 
        url: '/photos/annecy/street.JPG', 
        alt: 'Street View',
        caption: 'Rule of thirds in the streets of Annecy.'
      },
      { 
        id: 7, 
        url: '/photos/annecy/backyard.JPG', 
        alt: 'Airbnb Backyard',
        caption: 'The Airbnb was super cool. Nice backyard.'
      },
      { 
        id: 8, 
        url: '/photos/annecy/city.JPG', 
        alt: 'Annecy City',
        caption: 'We had some really good crepes here.'
      },
      { 
        id: 9, 
        url: '/photos/annecy/lake.JPG', 
        alt: 'Lac Annecy',
        caption: 'I think I could live here if I spoke French.'
      }
    ]
  },
  chamonix: {
    name: 'Chamonix',
    description: '7.4 - 7.6.25 | Weekend trip with IRES cohort. 7.24.25 | So good I had to come back',
    photos: [
      { 
        id: 1, 
        url: '/photos/chamonix/peak.JPG', 
        alt: 'Aiguille du Midi',
        caption: 'Aiguille du Midi peeks over the clouds.'
      },
      { 
        id: 2, 
        url: '/photos/chamonix/street.JPG', 
        alt: 'Street View',
        caption: 'Great truffle pizza at JOIA with Mont Blanc in the background.'
      },
      { 
        id: 3, 
        url: '/photos/chamonix/rocks.JPG', 
        alt: 'Rocks and Paraglider',
        caption: 'We took the cable car up to Planpraz and hiked up to Brevent.'
      },
      { 
        id: 4, 
        url: '/photos/chamonix/hiker_1.JPG', 
        alt: 'Random Hiker',
        caption: 'Random hiker looking aesthetic against the mountains.'
      },
      { 
        id: 5, 
        url: '/photos/chamonix/hiker_2.JPG', 
        alt: 'Random Hiker Again',
        caption: 'Same random hiker walking a little further.'
      },
      { 
        id: 6, 
        url: '/photos/chamonix/henry.JPG', 
        alt: 'Henry',
        caption: 'Not a random hiker. This is Henry taking a break.'
      },
      { 
        id: 7, 
        url: '/photos/chamonix/cable_1.JPG', 
        alt: 'Cable Car',
        caption: 'Cable car to Brevent looking so scary.'
      },
      { 
        id: 8, 
        url: '/photos/chamonix/moon.JPG', 
        alt: 'Moon',
        caption: 'Beautiful sunset against the mountain and moon.'
      },
      { 
        id: 9, 
        url: '/photos/chamonix/aynaz_1.JPG', 
        alt: 'Aynaz',
        caption: 'I think Aynaz is facing the wrong way.'
      },
      { 
        id: 10, 
        url: '/photos/chamonix/cabin.JPG', 
        alt: 'Cabin',
        caption: 'Cozy looking cabin on the hike to Brevent.'
      },
      { 
        id: 11, 
        url: '/photos/chamonix/spout.JPG', 
        alt: 'Spout',
        caption: 'Water spout at Chalet de La Floria. Highly recommend.'
      },
      { 
        id: 12, 
        url: '/photos/chamonix/aynaz_2.JPG', 
        alt: 'Aynaz Again',
        caption: 'I think Aynaz is facing the wrong way again.'
      },
      { 
        id: 13, 
        url: '/photos/chamonix/glacier.JPG', 
        alt: 'Mer de Glace',
        caption: 'The Mer de Glace glacier is almost gone. We need to take care of the world.'
      },
      { 
        id: 14, 
        url: '/photos/chamonix/train.JPG', 
        alt: 'Train',
        caption: 'We took the train up to the glacier and hiked back down.'
      },
      { 
        id: 15, 
        url: '/photos/chamonix/cable_2.JPG', 
        alt: 'Cable Car Again',
        caption: 'One more cable car shot :)'
      }
    ]
  },
  geneva: {
    name: 'Geneva',
    description: '5.16 - 5.17.25, 7.30 - 7.31.25 | Flew in/out of Geneva to Lyon',
    photos: [
      // 
    ]
  },
  grenoble: {
    name: 'Grenoble',
    description: '5.25.25 | Day trip to Grenoble with the gang',
    photos: [
      // 
    ]
  },
  nice: {
    name: 'Nice',
    description: '7.19 - 7.21.25 | Nice for Sarah\'s birthday',
    photos: [
      // 
    ]
  },
  paris: {
    name: 'Paris',
    description: '6.6 - 6.7.25 | Visited Charles and Roland Garros. 7.14.25 | La Fete Nationale',
    photos: [
      // 
    ]
  },
  turin: {
    name: 'Turin',
    description: '6.20 - 6.22.25 | There are many churches in Turin',
    photos: [
      // 
    ]
  },
  london: {
    name: 'London',
    description: '1.7 - 5.1.25 | Study Abroad Spring 2026 in London',
    photos: [
      {
        id: 1,
        url: '/photos/london/london/evan_cathedral.JPG',
        alt: 'Evan and Southwark Cathedral',
        caption: 'Evan and Southwark Cathedral on the Notre Dame Big Bus Tour.'
      },
      {
        id: 2,
        url: '/photos/london/london/bigben_1.JPG',
        alt: 'Big Ben Dramatic',
        caption: 'Big Ben from the National Gallery in Trafalgar Square on the way to class.'
      },
      {
        id: 3,
        url: '/photos/london/london/bigben_2.JPG',
        alt: 'Big Ben Cinematic',
        caption: 'Another shot of Big Ben from Trafalgar Square.'
      },
      {
        id: 4,
        url: '/photos/london/london/george4.JPG',
        alt: 'George IV Equestrian',
        caption: 'George IV Equestrian statue in Trafalgar Square.'
      },
      {
        id: 5,
        url: '/photos/london/london/nelson_column.JPG',
        alt: 'Nelson\'s Column',
        caption: 'Nelson\'s Column in Trafalgar Square on the way to class.'
      },
      {
        id: 6,
        url: '/photos/london/london/trafalgar_crowd.JPG',
        alt: 'Trafalgar Crowd',
        caption: 'Crowded Trafalgar Square in front of National Gallery.'
      },
      {
        id: 7,
        url: '/photos/london/london/evan_radios.JPG',
        alt: 'Evan and Radios',
        caption: 'Evan and radios in the Tate Modern.'
      },
      {
        id: 8,
        url: '/photos/london/london/radios.JPG',
        alt: 'Radios',
        caption: 'Tower of radios in the Tate Modern.'
      },
      {
        id: 9,
        url: '/photos/london/london/yellow_stairs.JPG',
        alt: 'Yellow Stairs',
        caption: 'Yellow stairs lead where?'
      },
      {
        id: 10,
        url: '/photos/london/london/rbo_hall.JPG',
        alt: 'Royal Ballet and Opera',
        caption: 'Royal Ballet and Opera Hall.'
      },
      {
        id: 11,
        url: '/photos/london/london/figaro.JPG',
        alt: 'Pink Nissan Figaro',
        caption: 'Pink car parked on the street. Identified a Nissan Figaro by Landon.'
      },
      {
        id: 12,
        url: '/photos/london/london/gc.JPG',
        alt: 'GC Eating Sandwich',
        caption: 'GC eating a fire pork sandwich from Borough Market.'
      },
      {
        id: 13,
        url: '/photos/london/london/chinatown.JPG',
        alt: 'So Flyday',
        caption: 'Chinatown on the walk back to Fischer Hall.'
      },
      {
        id: 14,
        url: '/photos/london/london/eye.JPG',
        alt: 'London Eye',
        caption: 'The London Eye on a sunny afternoon.'
      },
      {
        id: 15,
        url: '/photos/london/london/marbles.JPG',
        alt: 'Parthenon Marbles',
        caption: 'Parthenon Marbles in the British Museum.'
      },
    ]
  },
  leeds: {
    name: 'Leed\'s Castle',
    description: '1.17.26 | Class trip to Leed\'s Castle',
    photos: [
      {
        id: 1,
        url: '/photos/london/leeds/castle.JPG',
        alt: 'Leed\'s Castle Side View',
        caption: 'Leed\'s Castle was home to six English queens and King Henry VIII.'
      },
      {
        id: 2,
        url: '/photos/london/leeds/entrance.JPG',
        alt: 'Leed\'s Castle Entrance',
        caption: 'Going over the moat. Evan and Pedro led the way.'
      },
      {
        id: 3,
        url: '/photos/london/leeds/cabin.JPG',
        alt: 'Cabin',
        caption: 'I thought this cabin across the water in front of the castle was cool.'
      },
      {
        id: 4,
        url: '/photos/london/leeds/nook.JPG',
        alt: 'Leed\'s Castle Room',
        caption: 'A room inside the castle. It is said to be the loveliest in the world.'
      },
      {
        id: 5,
        url: '/photos/london/leeds/mirror.JPG',
        alt: 'Mirror',
        caption: 'There are many vintage items throughout the rooms, including some designer brands. Many birds, too.'
      },
      {
        id: 6,
        url: '/photos/london/leeds/stroll.JPG',
        alt: 'Stroll Through Gardens',
        caption: 'Stroll through the gardens on the castle grounds.'
      },
      {
        id: 7,
        url: '/photos/london/leeds/willow.JPG',
        alt: 'Willow Tree',
        caption: 'Willow tree on the pond.'
      },
      {
        id: 8,
        url: '/photos/london/leeds/maze.JPG',
        alt: 'Maze',
        caption: 'We couldn\'t find the crypt at the end. Some tried to mislead us.'
      },
      {
        id: 9,
        url: '/photos/london/leeds/hill.JPG',
        alt: 'Segways on the Hill',
        caption: 'People riding segways on the hill. We got lost somewhere on the acreage on the way back.'
      }
    ]
  },
  canterbury: {
    name: 'Canterbury',
    description: '1.17.26 | Class trip to Canterbury',
    photos: [
      {
        id: 1,
        url: '/photos/london/canterbury/front.JPG',
        alt: 'Canterbury Cathdral Front',
        caption: 'Entering Canterbury Cathedral grounds through the arch.'
      },
      {
        id: 2,
        url: '/photos/london/canterbury/cathedral.JPG',
        alt: 'Canterbury Cathedral Landscape',
        caption: 'Canterbury Cathedral is the seat of the Archbishop of Canterbury. It is the head of the Anglican Church of England.'
      },
      {
        id: 3,
        url: '/photos/london/canterbury/worm.JPG',
        alt: 'Cabin',
        caption: 'Entering Canterbury Cathedral. It was established in 597 and contains many old artifacts.'
      },
      {
        id: 4,
        url: '/photos/london/canterbury/altar.JPG',
        alt: 'Cathedral Altar',
        caption: 'Inside the Cathedral, facing the altar from the bishop\'s seats. The low lighting was beautiful but I couldn\'t keep my hand still.'
      },
      {
        id: 5,
        url: '/photos/london/canterbury/houses.JPG',
        alt: 'Street Houses',
        caption: 'These buildings remind me of German or Swiss architecture. I like the colors.'
      },
      {
        id: 6,
        url: '/photos/london/canterbury/library.JPG',
        alt: 'Public Library',
        caption: 'Intricate wood and glass detail on the library.'
      }
    ]
  },
  stonehenge: {
    name: 'Stonehenge',
    description: '1.30.26 | Class trip to Stonehenge',
    photos: [
      {
        id: 1,
        url: '/photos/london/stonehenge/stonehenge.JPG',
        alt: 'Stonehenge',
        caption: 'Stonehenge view from the Heel Stone (not pictured).'
      },
      {
        id: 2,
        url: '/photos/london/stonehenge/me.JPG',
        alt: 'Me at Stonehenge',
        caption: 'Me at Stonehenge, courtesy of Jane Kim.'
      },
      {
        id: 3,
        url: '/photos/london/stonehenge/vertical.JPG',
        alt: 'Stonehenge Vertical',
        caption: 'A closer view of the sarsen stones from the visitor path.'
      },
      {
        id: 4,
        url: '/photos/london/stonehenge/sheep.JPG',
        alt: 'Sheep',
        caption: 'Everyone wanted to see sheep.'
      },
      {
        id: 5,
        url: '/photos/london/stonehenge/couple.JPG',
        alt: 'Couple',
        caption: 'Walking up to Stonehenge along the Cursus from the visitor center.'
      },
      {
        id: 6,
        url: '/photos/london/stonehenge/prof.JPG',
        alt: 'Professor',
        caption: 'My professor aura farming.'
      }
    ]
  },
  bath: {
    name: 'Bath',
    description: '1.30.26 | Class trip to Roman Bath',
    photos: [
      {
        id: 1,
        url: '/photos/london/bath/entrance.JPG',
        alt: 'Entrance',
        caption: 'Entrance to the Roman Baths musuem.'
      },
      {
        id: 2,
        url: '/photos/london/bath/circus.JPG',
        alt: 'Bath Circus',
        caption: 'Bath Circus close up shot. It was very rainy and I couldn\'t get a good wide shot :('
      },
      {
        id: 3,
        url: '/photos/london/bath/abbey.JPG',
        alt: 'Bath Abbey',
        caption: 'Bath Abbey in front of the Roman Baths.'
      },
      {
        id: 4,
        url: '/photos/london/bath/singer.JPG',
        alt: 'Country Singer',
        caption: 'A country singer near the Baths, much appreciated by Notre Dame students.'
      },
      {
        id: 5,
        url: '/photos/london/bath/bath.JPG',
        alt: 'Bath',
        caption: 'The Great Bath, dedicated to Aquae Sulis. It would have been nice to bathe here.'
      },
      {
        id: 6,
        url: '/photos/london/bath/bridge.JPG',
        alt: 'Pulteney Bridge',
        caption: 'Dao Wei in front of Pulteney Bridge with the hills in the background. The view was even better driving into the city midday.'
      }
    ]
  },
  oxford: {
    name: 'Oxford',
    description: '2.1.26 | Day trip to Oxford',
    photos: [
      {
        id: 1,
        url: '/photos/london/oxford/camera_left.JPG',
        alt: 'Radcliffe Camera, Left',
        caption: 'Radcliffe Camera, a University of Oxford library. Camera means \'room\' in latin.'
      },
      {
        id: 2,
        url: '/photos/london/oxford/camera_mid.JPG',
        alt: 'Radcliffe Camera, Middle',
        caption: 'Radcliffe Camera.'
      },
      {
        id: 3,
        url: '/photos/london/oxford/camera_right.JPG',
        alt: 'Radcliffe Camera, Right',
        caption: 'Radcliffe Camera.'
      },
      {
        id: 4,
        url: '/photos/london/oxford/bridge.JPG',
        alt: 'The Bridge of Sigh',
        caption: 'The Bridge of Sighs.'
      },
      {
        id: 5,
        url: '/photos/london/oxford/christ_church_1.JPG',
        alt: 'Christ Church Gardens',
        caption: 'Gardens in front of Christ Church college.'
      },
      {
        id: 6,
        url: '/photos/london/oxford/christ_church_2.JPG',
        alt: 'Christ Church Front',
        caption: 'Christ Church was making us pay to go in. We didn\'t.'
      },
      {
        id: 7,
        url: '/photos/london/oxford/cinnamon.JPG',
        alt: 'Cinnamon Rolls',
        caption: 'Cinnamon rolls at a bakery we didn\'t get to try. They look so yummy.'
      },
      {
        id: 8,
        url: '/photos/london/oxford/reading.JPG',
        alt: 'The Norrington Room',
        caption: 'The Norrington Room in the famous bookstore, Blackwell\'s.'
      },
      {
        id: 9,
        url: '/photos/london/oxford/street.JPG',
        alt: 'Street',
        caption: 'A cool street on the way to the station.'
      },
      {
        id: 10,
        url: '/photos/london/oxford/chapel.JPG',
        alt: 'Brasenose Chapel',
        caption: 'Brasenose Chapel.'
      },
      {
        id: 11,
        url: '/photos/london/oxford/fuzz.JPG',
        alt: 'Fuzzy Brush',
        caption: 'Fuzzy looking brush growing on the walls.'
      },
      {
        id: 12,
        url: '/photos/london/oxford/narnia.JPG',
        alt: 'Narnia Door',
        caption: 'The Narnia Door on St. Mary\'s Passage, believed to have inspired C.S. Lewis.'
      }
    ]
  },
  thessaloniki: {
    name: 'Thessaloniki',
    description: '2.6 - 2.8.26 | Weekend in Greece with the boys',
    photos: [
      {
        id: 1,
        url: '/photos/london/thessaloniki/city_road.JPG',
        alt: 'City Road',
        caption: 'City apartment buildings all so compact. They look unique but very similar at the same time.'
      },
      {
        id: 2,
        url: '/photos/london/thessaloniki/city_vert.JPG',
        alt: 'City Vertical',
        caption: 'Another view of apartments.'
      },
      {
        id: 3,
        url: '/photos/london/thessaloniki/walk.JPG',
        alt: 'Man in Aristotle Square',
        caption: 'Man posing in Aristotle Square. He\'s so tuff.'
      },
      {
        id: 4,
        url: '/photos/london/thessaloniki/church.JPG',
        alt: 'Holy Church of Saint Paul the Apostle',
        caption: 'Holy Church of Saint Paul the Apostle.'
      },
      {
        id: 5,
        url: '/photos/london/thessaloniki/hilltop.JPG',
        alt: 'Ano Poli Hilltop',
        caption: 'Houses on the hilltop of Ano Poli, the old town.'
      },
      {
        id: 6,
        url: '/photos/london/thessaloniki/boys.JPG',
        alt: 'The Boys at the Heptapygrion',
        caption: 'The Boys at Heptapygrion.'
      },
      {  
        id: 7,
        url: '/photos/london/thessaloniki/mountain_view.JPG',
        alt: 'Mountain View',
        caption: 'View of the mountains from Heptapygrion.'
      },
      {  
        id: 8,
        url: '/photos/london/thessaloniki/ocean_view.JPG',
        alt: 'Ocean View',
        caption: 'View of the ocean from Heptapygrion.'
      },
      {  
        id: 9,
        url: '/photos/london/thessaloniki/car.JPG',
        alt: 'Car',
        caption: 'View of the houses and a car from Heptapygrion.'
      },
      {  
        id: 10,
        url: '/photos/london/thessaloniki/cat_1.jpg',
        alt: 'Cat 1',
        caption: 'Greek cat at Heptapygrion.'
      },
      {  
        id: 11,
        url: '/photos/london/thessaloniki/cat_2.JPG',
        alt: 'Cat 2',
        caption: 'Greek cat in alley of Ano Poli.'
      },
      {  
        id: 12,
        url: '/photos/london/thessaloniki/cat_3.JPG',
        alt: 'Cat 3',
        caption: 'Greek cat at Trigonion Tower.'
      },
      {  
        id: 13,
        url: '/photos/london/thessaloniki/pirate.JPG',
        alt: 'Pirate Ship',
        caption: 'Pirate ship on the sea.'
      },
      {  
        id: 14,
        url: '/photos/london/thessaloniki/dog.JPG',
        alt: 'Dog',
        caption: 'Dog is happy to be walking in Greece.'
      },
      {  
        id: 15,
        url: '/photos/london/thessaloniki/tower.JPG',
        alt: 'White Tower',
        caption: 'The White Tower.'
      },
      {  
        id: 16,
        url: '/photos/london/thessaloniki/sunlit_1.JPG',
        alt: 'Sunlit Buildings',
        caption: 'Sunset at our last dinner.'
      },
      {  
        id: 17,
        url: '/photos/london/thessaloniki/sunlit_2.JPG',
        alt: 'Sunlit Buildings Again',
        caption: 'Sunset at our last dinner, again.'
      },
      {  
        id: 18,
        url: '/photos/london/thessaloniki/sunset.JPG',
        alt: 'Sunset on the Sea',
        caption: 'Sunset on the sea before getting asked if we know BTS, and getting some fire crepes and loukoumades.'
      }
    ]
  },
  seven_sisters:{
    name: 'Seven Sisters',
    description: '2.20.26 | Hiking the chalk cliffs of Seven Sisters on the English Channel.',
    photos: [
      {
        id: 1,
        url: '/photos/london/seven_sisters/cliffs_1.JPG',
        alt: 'Cliffs Start',
        caption: 'Seven Sisters in South Downs National Park. The chalk cliffs are stark against the gray ocean and green grass.'
      },
      {
        id: 2,
        url: '/photos/london/seven_sisters/cliffs_2.JPG',
        alt: 'Cliffs End',
        caption: 'Cliffs from the visitor center near East Dean, going up to the lighthouse.'
      },
      {
        id: 3,
        url: '/photos/london/seven_sisters/cliffs_3.JPG',
        alt: 'Cliffs End',
        caption: 'We got drenched from the rain and wind pelting us from the ocean.'
      },
      {
        id: 4,
        url: '/photos/london/seven_sisters/rolling.JPG',
        alt: 'Rolling Hills',
        caption: 'There are seven hills that we climbed up and down along the cliffs. That\'s where the name comes from!'
      },
      {
        id: 5,
        url: '/photos/london/seven_sisters/visitor.JPG',
        alt: 'Visitor Center',
        caption: 'Visitor center near East Dean. Took the bus to Cuckmere river, and walked back 30 minutes from here to East Dean bus.'
      },
      {
        id: 6,
        url: '/photos/london/seven_sisters/lighthouse_1.JPG',
        alt: 'Lighthouse Boys',
        caption: 'The boys: Michael Morales, JR, Henry, and Dao Wei (not pictured).'
      },
      {
        id: 7,
        url: '/photos/london/seven_sisters/lighthouse_2.JPG',
        alt: 'Lighthouse Hikers',
        caption: 'The wind was so strong. We all got soaked and had to walk back wet and cold.'
      },
      {
        id: 8,
        url: '/photos/london/seven_sisters/sheep_1.JPG',
        alt: 'Sheep On Cliff',
        caption: 'Sheep got out of the fence. They were chilling near the cliff.'
      },
      {
        id: 9,
        url: '/photos/london/seven_sisters/sheep_2.JPG',
        alt: 'Sheep on Cliff',
        caption: 'Oh, to be a sheep, munching grass.'
      }
    ]
  },
  istanbul:{
    name: 'Istanbul',
    descrition: '2.26 - 3.2.26 | ',
    photos: [
      {}
    ]
  },
  madeira:{
    name: 'Madeira',
    descrition: '3.5 - 3.10.26 | ',
    photos: [
      {}
    ]
  },
  rome:{
    name: 'Rome',
    descrition: '3.10 - 3.11.26 | ',
    photos: [
      {}
    ]
  },
  pompeii:{
    name: 'Pompeii',
    descrition: '3.12.26 | ',
    photos: [
      {}
    ]
  },
  amalfi:{
    name: 'Amalfi Coast and Salerno',
    descrition: '3.13 - 3.15.26 | ',
    photos: [
      {}
    ]
  }
}

export const galleryNav = [
  { type: 'gallery', 
    id: 'home' 
  },
  { type: 'folder',
    id: 'study-abroad',
    name: 'Study Abroad',
    galleries: [
      'london',
      'leeds',
      'canterbury',
      'stonehenge',
      'bath',
      'oxford',
      'seven_sisters',
      'thessaloniki',
      'istanbul',
      'madeira',
      'rome',
      'pompeii',
      'amalfi'
    ]
  },
  { type: 'folder', 
    id: 'lyon-research',
    name: 'Summer Research Lyon',
    galleries: [
      'lyon',
      'annecy',
      'chamonix',
      'geneva',
      'grenoble',
      'nice',
      'paris',
      'turin'
    ] },
]